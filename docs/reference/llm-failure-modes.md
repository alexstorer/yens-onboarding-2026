...
- **Add format and sanity checks.** Cheap, automatic guards catch a surprising share of errors: validate structure and types with [Pydantic]({{ '/day1/extracting-data-with-an-llm/#step-6-validate-with-pydantic' | relative_url }}), as you did on Day 1, and check ranges and formats against real-world logic — a date that isn't a date, a negative probability, etc. Check the distribution of your outputs too: if you expect the data to be distributed a certain way and it isn't, that may be an indication something has gone wrong.
...
**First, make something worth protecting.** Run this at the repo root (`~/yens-onboarding-2026`):
